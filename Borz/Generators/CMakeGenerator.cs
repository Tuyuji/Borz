...
public class CMakeGenerator : IGenerator
{
    ...
    public void Generate()
    {
        ...
        var file = new System.IO.StreamWriter(Path.Combine(Workspace.Location, "CMakeLists.txt"), new FileStreamOptions(){Mode = FileMode.Create, Access = FileAccess.Write});
    ...
    private void GenerateProject(CProject project, ref StreamWriter file)
    {
        ...
        switch (project.Type)
        {
            ...
            case BinType.ConsoleApp:
                file.WriteLine($"add_executable({project.Name} {StrListToCMake(ProjectFilesToAbsolute(project, project.SourceFiles))})");
                break;
            case BinType.SharedObj:
                file.WriteLine($"add_library({project.Name} SHARED {StrListToCMake(ProjectFilesToAbsolute(project, project.SourceFiles))})");
                break;
            case BinType.StaticLib:
                file.WriteLine($"add_library({project.Name} STATIC {StrListToCMake(ProjectFilesToAbsolute(project, project.SourceFiles))})");
                break;
            default:
        ...
        if(project.PublicIncludePaths.Count > 0)
            file.WriteLine($"target_include_directories({project.Name} PUBLIC {StrListToCMake(ProjectFilesToAbsolute(project, project.PublicIncludePaths))})");
        
        if(project.PrivateIncludePaths.Count > 0)
            file.WriteLine($"target_include_directories({project.Name} PRIVATE {StrListToCMake(ProjectFilesToAbsolute(project, project.PrivateIncludePaths))})");
        ...
        //setup library paths
        if (project.LibraryPaths.Count > 0)
        {
